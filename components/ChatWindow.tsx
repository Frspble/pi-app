...
function phaseLabel(phase: AgentPhase, t: ReturnType<typeof useI18n>["t"]): string {
  if (phase?.kind === "running_tools") {
    ...
    if (names.length === 0) return t("chat.phase.runningTool");
    if (names.length === 1) return t("chat.phase.runningOne", { name: names[0] });
    if (names.length <= 3) return t("chat.phase.runningMany", { names: names.join(", ") });
    return t("chat.phase.runningMore", { names: names.slice(0, 2).join(", "), count: names.length - 2 });
  }
  if (phase?.kind === "waiting_model") return t("chat.phase.waitingModel");
  ...
  return (
    ...
    >
          ...
          <div className="w-full max-w-[820px]">
            ...
            >
              <div style={{ display: "flex", alignItems: "baseline", gap: 10, minWidth: 0, flex: 1, lineHeight: 1.4 }}>
                ...
                <span style={{ fontSize: 22, color: "var(--text)", fontWeight: 700, letterSpacing: "-0.01em" }}>Pi App</span>
      ...
      <div className="relative flex flex-1 overflow-hidden">
        <div ref={scrollContainerRef} className="chat-scroll-container flex-1 overflow-y-auto pt-4 [scrollbar-width:none]">
          <div className="mx-auto max-w-[820px] px-4">
            ...
            {streamState.isStreaming && streamState.streamingMessage && (
              <MessageView message={streamState.streamingMessage as AgentMessage} isStreaming modelNames={modelNames} cwd={messageBasePath} />
            )}
