...
function AddSkillPanel({
  ...
  const search = useCallback(async (q: string) => {
    ...
    try {
      ...
      if ((d.results ?? []).length === 0) setSearchError(t("skills.noSkillsFound"));
    } catch (e) {
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          >
            {(["global", "project"] as const).map((s) => (
              ...
              >
                {s === "global" ? t("skills.global") : t("skills.project")}
              </button>
            ))}
          </div>
          <span
      ...
      {results.length > 0 ? (
        ...
        !searching && (
          ...
          >
            ...
            {resolvedLanguage === "zh" ? "，为你的 Agent 发现并安装技能。" : " to discover and install skills for your agent."}
          </div>
        )
      )}
    </div>
                  ...
                  return groups.map(
                    ({ label: grpLabel, skills: grpSkills }) => (
                      <div key={grpLabel} style={{ marginBottom: 6 }}>
                        ...
                        >
	                          {grpLabel === "global" ? t("skills.global") : grpLabel === "project" ? t("skills.project") : grpLabel}
                        </div>
                        {grpSkills.map((skill) => {
                          const isSelected =
